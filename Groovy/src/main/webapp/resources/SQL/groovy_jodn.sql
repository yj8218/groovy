...
-- 날짜 차이 구하기
SELECT SUM(1) -- "총날수"
,SUM(DECODE(TO_CHAR(TO_DATE('20061201 09:00 AM','YYYY-MM-DD hh:mi AM')+LV-1,'DY'),'토',0,'일',0,1)) --"토/일 제외날수"
FROM (
SELECT LEVEL LV
FROM DUAL
CONNECT BY LEVEL<=TO_DATE('2006-12-20 09:00 AM','YYYY-MM-DD hh:mi AM')-TO_DATE('20061201','YYYYMMDD')+1
)
