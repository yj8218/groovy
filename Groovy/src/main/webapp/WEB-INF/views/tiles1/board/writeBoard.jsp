		  ...
		  <%-- === #166. 스마트 에디터 구현 시작 === --%>
		    ...
		    <%-- === 스마트 에디터 구현 끝 === --%>
			  ...
			  $("button#btnWrite").click(function(){
				  ...
				  // 글제목 유효성 검사
				  const subject = $("input#b_subject").val().trim();
				  if(subject == "") {
					  alert("글제목을 입력하세요!!");
					  return;
				  }
	...
	<table style="width: 1024px" class="table table-bordered">
		...
		<tr>
			...
			<td>
				<%-- == 원글쓰기 인 경우 == --%>
				<c:if test="${requestScope.fk_seq eq ''}">
				   <input type="text" name="b_subject" id="b_subject" size="100" /> 
				</c:if> 
				
				<%-- == 답변글쓰기 인 경우 == --%>
				<c:if test="${requestScope.fk_seq ne ''}">
				   <input type="text" name="b_subject" id="b_subject" size="100" value="${requestScope.b_subject}" readonly />  
				</c:if>	 
			</td>
		</tr>
		...
		<tr>
			...
			<td>
				<textarea style="width: 100%; height: 612px;" name="b_content" id="content"></textarea>
			</td>
		</tr>
		...
		<tr>
			...
			<td>
				<input type="password" name="b_pw" id="pw" /> 
			</td>
		</tr>
	</table>
